...
export function VideoPlayer({
  ...
  useEffect(() => {
    ...
    const checkTime = (): void => {
      const videoEl = player.get?.() as HTMLVideoElement | undefined
      if (!videoEl) return
      ...
      if (currentTime > 0 && duration > 0) {
        if (onTimeUpdate && currentTime - lastTimeUpdate.current >= 1) {
          lastTimeUpdate.current = currentTime
          onTimeUpdate(currentTime, duration)
        }

        if (onEnded && videoEl.ended) {
        ...
        <Video
          ...
          onEnded={() => {
            if (onEnded) {
              const videoEl = player.get?.() as HTMLVideoElement | undefined
              if (videoEl) {
                onEnded(videoEl.currentTime || 0, videoEl.duration || 0)
              }
            }
          }}
        >
...
function AudioTrackButton(): ReactNode {
  const player = usePlayer() as any
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const playerEl = player.get?.()
      if (playerEl?.audioTracks) {
        const tracks = Array.from(playerEl.audioTracks) as any[]
        setAudioTracks(tracks)
        const selected = tracks.find((t: any) => t.selected)
  ...
  const handleTrackSelect = (trackId: string): void => {
    const playerEl = player.get?.()
    if (playerEl?.audioTracks) {
      const track = Array.from(playerEl.audioTracks).find((t: any) => t.id === trackId)
      if (track) {
        playerEl.audioTracks.selectedTrack = track
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen} side="top">
      <Popover.Trigger
        render={
          <Button className="media-button--audio" aria-label="Audio tracks">
            <svg
            ...
            >
              <path
                fill="currentColor"
                d="M9 3a1 1 0 0 1 1 1v4.17a2.58 2.58 0 0 0-.36.83h-1.28a2.58 2.58 0 0 0-.36-.83V4a1 1 0 0 1 1-1z"
              />
              <path
                fill="currentColor"
                d="M12.5 6.5a.5.5 0 0 1 .5.5v3a.5.5 0 0 1-1 0v-3a.5.5 0 0 1 .5-.5z"
              />
              <path
                fill="currentColor"
                d="M14.5 4a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0v-6a.5.5 0 0 1 .5-.5z"
              />
              <path
                fill="currentColor"
                d="M10.5 5a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2a.5.5 0 0 1 .5-.5z"
              />
              <path fill="currentColor" d="M9 12.5A3.5 3.5 0 0 1 5.5 9 3.5 3.5 0 0 1 9 5.5V12.5z" />
            </svg>
          </Button>
        }
      />
      ...
      >
        <div style={{ fontSize: '12px', color: '#888', marginBottom: '8px' }}>Audio Track</div>
        {audioTracks.map((track: any) => (
          <button
            ...
            style={{
              ...
              width: '100%',
              padding: '8px 12px',
              background: track.id === currentTrackId ? 'rgba(99, 102, 241, 0.2)' : 'transparent',
              border: 'none',
              color: track.id === currentTrackId ? '#fff' : '#ccc',
              cursor: 'pointer',
              textAlign: 'left',
              borderRadius: '4px',
              marginBottom: '2px'
...
function CastEnterIcon(props: ComponentProps<'svg'>): ReactNode {
  return (
    <svg
      ...
      {...props}
    >
      <path fill="currentColor" d="M.75 13.25c1.38 0 2.5 1.085 2.5 2.5V16H.5v-2.75z" />
      <path
        fill="currentColor"
  ...
  return (
    ...
    >
      ...
      <path
        ...
        d="M17.5 14.25c0 .963-.787 1.75-1.75 1.75h-5.5v-2h5.25V4h-13v2.25h-2v-2.5C.5 2.787 1.287 2 2.25 2h13.5c.963 0 1.75.787 1.75 1.75z"
      />
      <path fill="currentColor" d="M4 5.5v1.164c2.143-.021 5.979 3.836 5.979 5.979H14V5.5z" />
    </svg>
  )
}
