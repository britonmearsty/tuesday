...
const MoviesPageSkeleton = (): React.JSX.Element => (
...
function MoviesPage(): React.JSX.Element {
    ...
    async (pageNum: number) => {
      ...
      try {
        ...
        if (filters.genres.length > 0) {
          result = await tmdbApi.discoverMovies({
            page: pageNum,
            genres: filters.genres,
            sortBy: ['now_playing', 'upcoming'].includes(filters.sortBy)
              ? 'popularity.desc'
              : filters.sortBy
          })
        } else {
